...
func (p *Pane) Kill() {
	if p.Cmd != nil && p.Cmd.Process != nil {
		...
		// greps, etc.) die together.
		go p.killGroupAsync(p.Cmd.Process.Pid)
	}
	if p.Tty != nil {
		p.Tty.Close()
	}
...
// killGroupAsync sends SIGTERM to the child's process group, waits up
// to 2s for the leader to be reaped (via waitLoop closing p.exited),
// then SIGKILLs the group if anything is still alive. Must be invoked
// in its own goroutine — the caller (Kill) is on the
// DeletePane/RemoveProject hot path and can't block on a 2s grace.
//
// Group resolution: syscall.Getpgid(pid). This is the leader's pgid
// (== leader pid because Setsid was used at spawn). We address `-pgid`
// to send to the whole group. Failures from Getpgid (kid already
// reaped, race with Wait) are non-fatal — we fall back to a
// best-effort kill of the leader pid alone, matching the pre-fix
// behaviour.
func (p *Pane) killGroupAsync(pid int) {
	p.mu.Lock()
	exited := p.exited
	p.mu.Unlock()
	...
	if err != nil {
		// Race: child already reaped between Kill and Getpgid. Try a
		// direct-pid SIGKILL anyway in case Getpgid returned ESRCH for
		// some other reason. Errors here are swallowed.
		_ = syscall.Kill(pid, syscall.SIGKILL)
		return
	}
	// SIGTERM the whole group first. Negative pid → group address.
	_ = syscall.Kill(-pgid, syscall.SIGTERM)
	// Wait for the leader to be reaped (via Cmd.Wait in waitLoop
	// closing p.exited). If it doesn't reap within 2s — meaning either
	// the leader trapped TERM or grandchildren are still bringing the
	// daemon's wait through — escalate to SIGKILL.
	if exited == nil {
		// Defensive: partially-constructed test stub. Skip the wait.
		return
	}
	select {
	case <-exited:
	...
	case <-time.After(2 * time.Second):
		// SIGKILL is uncatchable; leader must die. Grandchildren reparent
		// to init and the kernel reaps them.
