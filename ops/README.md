...
Run `v2/ops/install-satellite.sh` on your laptop. It installs FUSE-T +
sshfs, generates an SSH key, wires a `reck-station` alias into
`~/.ssh/config`, and registers a LaunchAgent (`eu.verwey.reck-mount`)
that keeps `~/reck/projects/` mounted from the station's
`/Users/reck-connect/projects/` via Tailscale.
