...
sed "s|__HOME__|${home_esc}|g" "$PLIST_SRC" > "$plist_tmp"
...
# winner's intact.
if [[ -n "$WRITE_SETTINGS_URL" ]]; then
  APP_DATA_DIR="$HOME/Library/Application Support/Reck Connect Satellite"
  BOOTSTRAP_FILE="$APP_DATA_DIR/bootstrap.json"
  echo "==> Seeding Satellite first-launch config"
...
PY
  ...
  echo "     Satellite first launch will import this and delete the file."
  echo "     If 'settings' is already populated, the import is a no-op."
fi
...
if (( KEY_ALREADY_INSTALLED )); then
  ...
  echo
  echo "  ssh-copy-id -i $KEY.pub reck-connect@$STATION_HOST"
  echo
  echo "Run that to register the mount key on the station."
fi
echo "Once the station also has \$(install-station.sh) applied (sentinel"
echo "file created under /Users/reck-connect/projects/), the watchdog will"
