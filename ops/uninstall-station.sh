...
# Issue #225: reck-pane-launcher is the TCC responsible-process helper.
# Removing the binary does NOT clear the user's Accessibility grant —
# TCC.db keys grants on the binary path; the entry stays in System
# Settings → Privacy & Security → Accessibility until manually removed.
sudo rm -f /usr/local/bin/reck-pane-launcher
