...
/**
 * Canonical label of the station's LaunchDaemon, per
 * `/Library/LaunchDaemons/eu.verwey.reck-stationd.plist`. We look up the
 * station daemon's live pid by label (not by `pid/<pid>`) so that (a)
 * the probe works from an unprivileged user context without sudo — a
 * concern flagged during review since the system-domain job is queried
 * as root by `install-station.sh`, and (b) we only false-positive on
 * the one pid that actually is the station daemon, not on any random
 * launchd-tracked process that shares a port.
 */
const STATION_LAUNCHDAEMON_LABEL = "system/eu.verwey.reck-stationd";

/**
 * Return the currently-running pid of the station LaunchDaemon, or null
 ...
 *
 * The station LaunchDaemon binds `0.0.0.0:7315`, so a local-mode
...
function defaultStationDaemonPid(): number | null {
  let out = "";
  try {
    out = execFileSync("/bin/launchctl", ["print", STATION_LAUNCHDAEMON_LABEL], {
      stdio: ["ignore", "pipe", "ignore"],
      encoding: "utf8",
      timeout: 2000,
    });
...
// LaunchDaemon. Orphans show up when a previous Satellite launch
...
// port is still taken.
//
// The station-LaunchDaemon guard protects the supervised `system/
// eu.verwey.reck-stationd` process from being killed by a local-mode
// Satellite running on the same host; see defaultStationDaemonPid for
// the rationale and the 2026-04-24 incident that motivated this code.
function killOrphanedDaemons(
  ...
  for (const pid of listPids(port)) {
    ...
    if (stationPid !== null && pid === stationPid) {
      console.log(`[reck-stationd] skip kill pid=${pid} (station LaunchDaemon)`);
      continue;
    }
    try {
      signalPid(pid);
...
export interface StartDaemonDeps {
  ...
  /** Override the "what pid is the station LaunchDaemon running as?"
  ...
  const child = spawnFn(
    ...
    ],
    {
      ...
      env: {
        ...
        DAEMON_TOKEN: token,
        // Match the station LaunchDaemon plist so panes spawned by the
...
// Intentional: we do NOT kill the daemon on window close or app quit. The
// daemon is only stopped when the user explicitly picks "Quit Daemon…" from
// the File menu. Keeping this hook as a placeholder documents that design.
app.on("will-quit", () => {
