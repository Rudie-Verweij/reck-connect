...
describe("killOrphanedDaemons — station LaunchDaemon is skipped", () => {
  // Regression for the 2026-04-24 station bounce: the orphan sweep would
  // SIGTERM the station LaunchDaemon when a local-mode Satellite started
  // on the same host, because lsof matched on process name alone. The
  // fix threads a `launchctl print system/eu.verwey.reck-stationd`
  // lookup through the sweep and skips exactly that pid.
  //
  ...
  it("does NOT signal the station LaunchDaemon pid", async () => {
    const spawn = makeSpawnRecorder();
  ...
  it("partitions mixed pids: skip the station pid, signal the orphan", async () => {
    ...
    // Only the orphan (9002) gets a SIGTERM; the LaunchDaemon (9001)
  ...
  it("falls back to killing everything when station daemon pid can't be resolved", async () => {
    ...
    // registered the station LaunchDaemon would be unable to recover
