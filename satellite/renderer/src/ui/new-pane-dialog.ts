import type { PaneKind, RestoreCandidateGroup, SessionInfo } from "@proto/proto";
...
export type RestorePromptChoice = "restore" | "skip" | null;

/**
 * "The station restarted — restore 3 sessions?" prompt shown on
 * reconnect when the daemon reports orphan-live sessions. Deliberately
 * low-detail: two buttons (Restore all / Skip), a summary of what
 * would be brought back. Picking individual sessions is a follow-up.
 */
export function askRestoreSessions(
  parent: HTMLElement,
  candidates: RestoreCandidateGroup[],
): Promise<RestorePromptChoice> {
  return new Promise((resolve) => {
    const total = candidates.reduce((n, g) => n + g.sessions.length, 0);
    if (total === 0) {
      resolve(null);
      return;
    }
    const breakdown = candidates
      .map((g) => {
        const rows = g.sessions
          .map((s) => {
            // an earlier release Scope B: session_id is Claude-only, slot_id is
            // shell-only. Fall back to whichever the row carries so we
            // always have something to display for anonymous rows.
            const fallback = (s.session_id || s.slot_id || "").slice(0, 8);
            const label = s.name || fallback || (s.kind === "shell" ? "shell" : "claude");
            return `
              <li>
        ...
        return `
          <div class="restore-group">
            <div class="restore-group-title">${escapeHtml(g.project_name)}</div>
            <ul class="restore-group-list">${rows}</ul>
          </div>`;
      })
      .join("");
    const overlay = document.createElement("div");
    overlay.className = "new-pane-dialog";
    overlay.innerHTML = `
      <div class="options" role="alertdialog" aria-label="Restore sessions" style="max-width:520px;">
        <div class="dialog-title">The station restarted</div>
        <div class="dialog-body">
          ${total} pane${total === 1 ? " was" : "s were"} running when the
          station last stopped. Restore ${total === 1 ? "it" : "them"}?
        </div>
        <div class="restore-groups">${breakdown}</div>
        <div class="dialog-buttons" style="margin-top:1rem;">
          <button data-action="skip">Skip</button>
          <button class="primary" data-action="restore">Restore</button>
        </div>
      </div>
    `;
    parent.appendChild(overlay);
    const close = (result: RestorePromptChoice) => {
      overlay.remove();
      window.removeEventListener("keydown", onKey, true);
      resolve(result);
    };
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape") { e.preventDefault(); e.stopPropagation(); close(null); }
      else if (e.key === "Enter") { e.preventDefault(); e.stopPropagation(); close("restore"); }
    };
    window.addEventListener("keydown", onKey, true);
    overlay.querySelectorAll<HTMLButtonElement>("button").forEach((btn) => {
      btn.addEventListener("click", () => close(btn.getAttribute("data-action") as RestorePromptChoice));
    });
    (overlay.querySelector("button.primary") as HTMLElement)?.focus();
  });
}
