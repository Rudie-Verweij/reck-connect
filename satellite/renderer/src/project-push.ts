 ...
 * pathway always takes the build-time default.
 *
 * Required `VITE_RECK_STATION_ROOT` env var at build time — Vite inlines
 * the value into the bundle. Throw on missing rather than silently
 * baking a wrong literal that produces null translations + vanishing
 * projects in the rail with no error.
 */
function requireViteEnv(name: string): string {
  const v = (import.meta.env as Record<string, string | undefined>)[name];
  if (!v) throw new Error(`${name} is required at build time (Vite env)`);
  return v;
}
export const STATION_MANAGED_ROOT = requireViteEnv("VITE_RECK_STATION_ROOT");
