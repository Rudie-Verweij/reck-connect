...
export async function boot(splash?: StartupSplashController) {
  ...
  const layout: PaneLayout = new PaneLayout({
    ...
    // capabilities.clipboard_image=true for this pane (Claude pane +
    // sidecar reachable), POST raw bytes to /clipboard-image. The
    // sidecar writes NSPasteboard, daemon writes 0x16 to the PTY,
    // Claude Code creates an [Image #N] chip — no path leaks into the
    // prompt text. On 503 (sidecar gone, ACK timeout) we fall back to
    // /uploads silently with a console.info breadcrumb.
    //
    // Phase 1 (fallback / shell panes / older daemons): POST
    // multipart to /uploads, receive an absolute path, type it into
    // the PTY. Identical to Older-phase-2 behaviour.
    //
    ...
    onPasteUpload: async (paneId, host, blob, mime) => {
      ...
      if (paneClipboardImage.get(paneId) === true) {
        try {
          ...
          // 503 from /clipboard-image — fall through to /uploads.
        } catch (err) {
          // Non-503 4xx/5xx — also fall through to /uploads. The chip
          // path is opportunistic; treat any hard failure as "try the
          // universal route" rather than escalating to the user.
          console.info("[paste-clipboard] error, falling back to /uploads", { paneId, err });
        }
      }
      const resp = await api.uploadFile(paneId, blob, mime);
      return { kind: "path", path: resp.path };
    },
  ...
  /**
   * Fired once after the first successful connect. Asks the daemon
   * which sessions were running at the last observation but aren't
   * now, then prompts the user to Restore all or Skip. Resumed panes
   * are spawned and the Satellite is reloaded so xterm + the layout
   * reconcile around the new pane IDs.
   */
  async function maybeOfferRestore() {
    ...
    let resp;
    try {
      // Opt into shell-kind restore candidates. A post-Scope-B daemon
      // will include shell rows; a pre-Scope-B daemon ignores the
      // ?kinds= query param and returns Claude-only either way.
      resp = await client.restoreCandidates(["claude", "shell"]);
    } catch (e) {
      console.error("restoreCandidates failed", e);
      return;
    }
    if (resp.candidates.length === 0) return;
    const choice = await askRestoreSessions(document.body, resp.candidates);
    if (choice === "skip") {
      // Clear was_live so we don't re-prompt on next reconnect. The
      // daemon's dismiss endpoint matches by identity (SessionID for
      // Claude, SlotID for shell) — we pass whichever the row carries.
      for (const g of resp.candidates) {
        try {
          const ids = g.sessions
            .map((s) => s.session_id || s.slot_id || "")
            .filter((x) => x !== "");
          await client.dismissSessions(g.project_id, ids);
        } catch (e) {
          console.error("dismissSessions failed", e);
        }
      }
      return;
    }
    if (choice !== "restore") return;
    // Respawn each session in its project. We deliberately keep this
    // sequential — claude panes spike CPU during startup and a parallel
    // burst just serializes at the shell level anyway.
    for (const g of resp.candidates) {
      for (const s of g.sessions) {
        const args = buildRestoreCreateArgs(s);
        if (!args) continue; // malformed row, skip
        try {
          await client.createPane(g.project_id, args.kind, args.opts);
        } catch (e) {
          console.error("restore createPane failed", {
            project: g.project_id,
            kind: args.kind,
            id: s.session_id || s.slot_id,
            e,
          });
        }
      }
    }
    // Reload so the saved split/tab layouts reconcile against the
    // freshly spawned pane IDs via the existing reconcile() path.
