...
| `satellite/main/daemon-spawn.ts` | `startDaemon`, `stopDaemon`, `isDaemonRunning`, `findDaemonBinary`; `will-quit` no-op (daemon intentionally left running) |
...
|---|---|
| `CLAUDE.md` | Codebase guidance for Claude Code agents; build commands; gotchas |
